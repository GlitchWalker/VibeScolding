...
* **Update & Download URLs:** To match the repository distribution structure, you must explicitly construct the `@updateURL` and `@downloadURL` using the exact filename of the script appended to the base raw GitHub path.
    ...
    * *Example:* For a script named `FilterVibes.user.js`, the tags must look exactly like this:
        // @updateURL   https://raw.githubusercontent.com/GlitchWalker/VibeScolding/refs/heads/main/UserScripts/FilterVibes.user.js
        // @downloadURL https://raw.githubusercontent.com/GlitchWalker/VibeScolding/refs/heads/main/UserScripts/FilterVibes.user.js
...
## 4. Firefox CSP and Xray Vision (Critical)
...
* **Xray Vision Isolation:** Firefox strictly isolates page scripts from extension scripts. When interacting with the page's `window` object, you only see the native, unmodified DOM properties.
    * **Reading:** Read variables from the page context using `unsafeWindow`.
    * **Writing Objects:** To share JSON or data objects from the script into the page context, you MUST use Firefox's `cloneInto()` API (e.g., `unsafeWindow.injectedData = cloneInto(myData, unsafeWindow)`).
    * **Exporting Functions:** To allow the page to call a script function, use `exportFunction()` (e.g., `exportFunction(myFunc, unsafeWindow, { defineAs: "myFunc" })`).
    * Standard assignment (`unsafeWindow.myVar = myObj`) will throw security permission errors.

## 5. Performance & Efficiency Constraints (Firefox-Specific)
To prevent extension lag and reduce main-thread blocking on heavy pages:
* **DOM Mutation Monitoring:** Avoid persistent `setInterval` loops for checking DOM changes. Use highly targeted `MutationObserver` instances instead. Always disconnect observers as soon as the target elements are successfully handled.
...
* **Asynchronous Storage Operations:** When utilizing `GM.getValue` or `GM.setValue`, process them concurrently via `Promise.all()` where appropriate instead of chaining successive await statements.
