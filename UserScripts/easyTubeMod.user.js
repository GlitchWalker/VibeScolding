...
// @version            4.0.3
// @author             2pixel (rewrite: SpiderMonkey/FireMonkey optimized + InteropDebug v2.0)
// @license            MIT
...
// @compatible         firefox   FireMonkey / Tampermonkey / Violentmonkey
...
// @downloadURL        https://update.greasyfork.org/scripts/561432/EasyTube%20V40%20%E2%80%94%20Ad%20Skip%20%20SponsorBlock%20%20HD%20Download%E2%AC%87%EF%B8%8F%F0%9F%9A%80%20%28No%20Lag%29.user.js
// @updateURL          https://update.greasyfork.org/scripts/561432/EasyTube%20V40%20%E2%80%94%20Ad%20Skip%20%20SponsorBlock%20%20HD%20Download%E2%AC%87%EF%B8%8F%F0%9F%9A%80%20%28No%20Lag%29.meta.js
// ==/UserScript==
...
/*
 * MODIFICATION NOTICE & ATTRIBUTION
 * This is a modified version of the script available at:
 * https://greasyfork.org/en/scripts/561432-easytube-v4-0-ad-skip-sponsorblock-hd-download-no-lag
 * It is licensed under the MIT License, identical to the original script.
 */
...
// USER CONFIGURATION
// ═══════════════════════════════════════════════════════════════════════════
const UIVisible = true; // Set to false to hide ALL interface elements (toasts remain active).
...
// ═══════════════════════════════════════════════════════════════════════════
(function patchYT() {
    Logger.log('Initializing Layer 0 InnerTube patches.');
    try {
        unsafeWindow.Object.defineProperty(unsafeWindow.Object.prototype, 'adPlacements', {
            get() { return undefined; },
            set() {
                unsafeWindow.Object.defineProperty(this, 'adPlacements', {
                    value: [], writable: true, configurable: true, enumerable: true,
                });
            },
            configurable: true,
        });
        Logger.log('Patched unsafeWindow.Object.prototype.adPlacements');
    } catch (e) { Logger.error('Failed to patch adPlacements:', e); }

    try {
        let _yipr;
        unsafeWindow.Object.defineProperty(unsafeWindow, 'ytInitialPlayerResponse', {
            get() { return _yipr; },
            set(v) {
                if (v) {
                    if (v.adPlacements) {
                        v.adPlacements = [];
                        Logger.log('Wiped adPlacements from ytInitialPlayerResponse');
                    }
                    if (v.auxiliaryUi && v.auxiliaryUi.messageRenderers) {
                        try {
                            v.auxiliaryUi.messageRenderers.enforcementMessageViewModel = undefined;
                            Logger.log('Wiped enforcementMessageViewModel');
                        } catch (e) { /* ignore */ }
                    }
                }
                _yipr = v;
    ...
    } catch (e) { Logger.error('Failed to patch ytInitialPlayerResponse:', e); }
})();

// ═══════════════════════════════════════════════════════════════════════════
// AD & UI CSS — Native GM_addStyle for CSP compliance
// ═══════════════════════════════════════════════════════════════════════════
GM_addStyle(`
    /* Native Hardware-Accelerated Fullscreen Hiding (Zero-CPU cost) */
    :fullscreen #et4_tog, :fullscreen #et4_panel,
    :-webkit-full-screen #et4_tog, :-webkit-full-screen #et4_panel,
    :-moz-full-screen #et4_tog, :-moz-full-screen #et4_panel {
...
const CFG = {
    ...
    sbCats: ['sponsor','preview','music_offtopic','exclusive_access'],
    sbTypes: ['skip','mute'],
};
...
// ═══════════════════════════════════════════════════════════════════════════
const S = {
    adEnabled:      GM_getValue('et4_ad',      true), // Defaulted true for UX
...
function save(key, val) {
    GM_setValue(key, val);
    Logger.log('Saved configuration state:', key, val);
}
...
function _doSkip() {
    ...
    if (btn) {
        Logger.log('Skippable ad detected. Executing skip.');
    ...
    if (video && adActive && !S.adSpeedActive) {
        Logger.log('Unskippable ad detected. Accelerating playback to 16x.');
    ...
    if (video && !adActive && S.adSpeedActive) {
        Logger.log('Ad finished. Restoring playback state.');
        video.playbackRate = 1;
        video.muted  = false;
    ...
    const p = getPlayer();
    if (p) {
        p.querySelectorAll('.ytp-ad-overlay-close-button, .ytp-ad-overlay-slot-close-button')
...
function dismissEnforcement() {
    for (const sel of ENFORCEMENT_SELS) {
        ...
        if (!modal) { continue; }
        Logger.warn('Anti-adblock enforcement detected. Attempting bypass.');
    ...
    try {
        const popup = unsafeWindow.yt?.config_?.openPopupConfig?.supportedPopups;
        if (popup && popup.adBlockMessageViewModel !== undefined) {
            popup.adBlockMessageViewModel = false;
        }
    } catch (e) { /* ignore */ }

    try {
        const d = unsafeWindow.ytcfg?.data_?.PLAYER_VARS;
        if (d) { d.ad3_module = '0'; }
    } catch (e) { /* ignore */ }
}

// ═══════════════════════════════════════════════════════════════════════════
// OBSERVERS (Native Event-Driven)
// ═══════════════════════════════════════════════════════════════════════════
let _adObserver   = null;
let _bodyObserver = null;
let _sbVideoAttached = null;
...
function setupObservers() {
    ...
    const wait = setInterval(() => {
        ...
        // Attach native SpiderMonkey optimized event listener for SponsorBlock
        const video = getVideo();
        if (video && video !== _sbVideoAttached) {
            if (_sbVideoAttached) { _sbVideoAttached.removeEventListener('timeupdate', _doCheckSB); }
            video.addEventListener('timeupdate', _doCheckSB);
            _sbVideoAttached = video;
            Logger.log('Attached native timeupdate listener for SponsorBlock.');
        }
        ...
        _bodyObserver = new MutationObserver(mutations => {
            for (const m of mutations) {
                for (const node of m.addedNodes) {
                    ...
                    if (
                        (node.tagName && node.tagName.includes('ENFORCEMENT')) ||
                        (node.id && node.id.includes('enforcement')) ||
        ...
        Logger.log('DOM Observers successfully initialized.');
    }, 300);
}
...
function fetchSB(videoId) {
    ...
    S.sbVideoId  = videoId;
    S.sbSegments = [];
    Logger.log(`Fetching SponsorBlock data for video: ${videoId}`);
    ...
    function parseSegments(text) {
        try {
            ...
            let segs = [];
            if (Array.isArray(data) && data[0] && data[0].segments) {
                const match = data.find(d => d.videoID === videoId);
                segs = match ? match.segments : [];
            } else if (Array.isArray(data) && data[0] && data[0].segment) {
                segs = data;
            }
            ...
            Logger.log(`Loaded ${S.sbSegments.length} SponsorBlock segments.`);
        } catch (e) {
            S.sbSegments = [];
            Logger.error('Failed to parse SponsorBlock data:', e);
        }
    }
            ...
            onload(r) {
                ...
                if (hashUrl) {
                    GM_xmlhttpRequest({
                        ...
                        onerror() { Logger.error('Direct API fallback failed.'); },
                    });
                }
            },
            onerror() {
        ...
        if (seg.actionType === 'mute') {
            if (inSeg) {
                if (S.sbMutedSeg !== seg.uuid) {
                    ...
                    Logger.log(`SponsorBlock Muted: [${seg.cat}]`);
                }
            ...
            Logger.log(`SponsorBlock Skipped: [${seg.cat}]`);
            break;
        }
    }
}
...
function setQuality() {
    ...
    try {
        ...
        if (player.getPlaybackQuality() !== best) {
            ...
            Logger.log(`Quality enforced to ${best}`);
        }
    } catch (e) { /* ignore */ }
}
...
function downloadVideo(videoId) {
    ...
    Logger.log(`Opening download portal for ${videoId}`);
...
function getTitle() {
    ...
    for (const s of sel) {
        ...
        const t = node && node.textContent ? node.textContent.trim() : null;
...
function onNavigate() {
    ...
    _navTimer = setTimeout(() => {
        ...
        S.videoId = vid;
        Logger.log(`Navigation detected. Video ID: ${vid || 'None'}`);
...
let _uiRaf = false;
function uiSync() {
    if (!UIVisible || _uiRaf) { return; }
    _uiRaf = true;
    requestAnimationFrame(() => {
        _uiRaf = false;
        const adN = document.getElementById('et4_ad_n');
        if (adN) { adN.replaceChildren(document.createTextNode(S.adCount.toString())); }

        const sbN = document.getElementById('et4_sb_n');
        if (sbN) { sbN.replaceChildren(document.createTextNode(S.sbCount.toString())); }
...
function initDrag(panel) {
    ...
    hdr.addEventListener('pointerdown', e => {
        ...
        try { hdr.setPointerCapture(pid); } catch (err) { /* ignore */ }
    }, { passive: true });
...
function bindEvents(panel, tog) {
    ...
    document.getElementById('et4_dl_btn').addEventListener('click', () => {
        ...
        if (vid) {
            ...
            toast('⚠ Open a video first', '#f57c00');
        }
    });
...
// ═══════════════════════════════════════════════════════════════════════════
function boot() {
    Logger.log('Boot sequence initiated.');
    ...
    if (UIVisible) {
        ...
        Logger.log('UI generation complete.');
    } else {
        Logger.log('UI generation bypassed via UIVisible flag.');
    }
